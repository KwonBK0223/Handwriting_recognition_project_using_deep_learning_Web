...
def model_summary():
    ...
    img = Image.open(image_path)
    st.image(img, width = 1000)
    st.write("* 이미지 데이터를 증강하기 위해 ImageDataGenerator 객체를 생성합니다.")
    ...
    st.write("* Flatten 층을 추가하여 2D 데이터를 1D로 변환합니다.")
    st.write("* Dense 층을 추가하여 전결합층을 구성합니다. 출력 뉴런의 개수는 클래스 수에 해당합니다.")
    st.write("* 소프트맥스 활성화 함수를 사용하여 클래스별 확률을 출력합니다.")
    st.write("* learning_rate는 모델이 학습할 때 사용하는 학습률을 의미합니다. ")
    st.write(" 학습률은 가중치 업데이트의 크기를 조절하는 역할을 합니다.")
    st.write("* batch_size는 한 번의 반복 단위로 처리되는 데이터의 개수를 의미합니다. ")
    st.write(" 작은 배치 크기는 메모리 사용량이 적지만 학습 속도가 느릴 수 있으며, ")
    st.write(" 큰 배치 크기는 학습 속도가 빠르지만 메모리 사용량이 많아질 수 있습니다.")
    st.write("* dropout_rate는 드롭아웃 층에서 뉴런을 제외시킬 비율을 의미합니다. ")
    st.write(" 드롭아웃은 과적합을 방지하기 위해 일부 뉴런을 임의로 비활성화시키는 기법입니다.")
    st.write("* model.compile() 함수는 모델을 컴파일하는 단계입니다. ")
    st.write(" 손실 함수(loss)로는 categorical_crossentropy를 사용하고, 옵티마이저(optimizer)로는 Adam을 사용합니다.")
    st.write(" 또한, 모델 평가를 위해 정확도(accuracy)를 지정합니다. ")
    st.write("* model.summary() 함수는 모델의 구조와 파라미터 개수 등을 요약하여 출력합니다. ")
    st.write(" 모델의 총 파라미터 개수와 각 층의 출력 크기 등을 확인할 수 있습니다.")
    
    st.write("### 7. 학습(Fit)")
    image_path = 'Image/7.fit.PNG'
    img = Image.open(image_path)    
    st.image(img, width = 1000)
    st.write("* X_train_array와 X_test_array는 학습 데이터와 테스트 데이터를 NumPy 배열로 변환한 것입니다.")
    st.write("* model.fit() 함수는 모델을 학습하는 단계입니다.    ")
    st.write(" 데이터 증강을 적용한 학습 데이터를 사용하여 모델을 학습합니다.")
    st.write(" datagen.flow() 함수를 통해 데이터를 증강하고, 이를 모델에 입력하여 학습합니다.")
    st.write(" steps_per_epoch는 한 epoch(전체 데이터에 대한 한 번의 학습 과정)당 배치 수를 의미하며, ")
    st.write(" epochs는 전체 데이터셋을 몇 번 반복하여 학습할지를 나타냅니다. validation_data는 검증 데이터를 의미하며,")
    st.write(" 학습 중간에 모델의 성능을 평가하는 용도로 사용됩니다.")
